...
For example, suppose we wanted to create a linear regression model. To initiate this model, we would call the [`linear_reg()`](https://parsnip.tidymodels.org/reference/linear_reg.html) function. 

```{r} 
lr <- linear_reg()
```

Alternatively, to initialize a random forest model we would use the [`rand_forest()`](https://parsnip.tidymodels.org/reference/rand_forest.html) function. 
...
* [**`set_engine()`**](https://parsnip.tidymodels.org/reference/set_engine.html) determines the model *engine*, or package that will handle model creation. For many models, there are multiple packages that can run that model, each with their own unique syntax and interface. `parsnip` regularizes the interface, but doesn't replace the modeling package -- meaning that we must specify which package we'd like to use. If later on we'd like to switch engines, simply change the argument for `set_engine()`. 
...
Once a model has been initialized and defined, we can execute the model on our prepped training data using the `fit()` function. `fit()` takes in a formula and a model object and returns a fitted model. 
