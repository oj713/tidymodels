...
When building a model, we usually want to estimate the performance of the model **before** we evaluate it with the testing set. Resampling encompasses various techniques to split the training set so that we can estimate model performance prior to polishing and assessing with the testing set. Learn to work with resamples in the [Tune](Tune.html) and [Workflowsets](WorkflowSets.html) tutorials. 
